...
program: FUNC MAIN LB RB START COLON { printf("#include<stdio.h>\nint main()\n{\n");  }
     PROGRAM_STATEMENTS END COLON { printf("\n}"); }
...
PROGRAM_STATEMENT: VAR_DECL DATA_TYPE VAR_LIST SC {
              ...
              | PRINT LB { printf("printf("); } TO_WRITE {printf("\b");} RB SC {
                printf("\"");
                for(int i=0; i<idx; i++)
                {
                  printf(",%s", var_list[i]);
	      ...
	      |IF_BLOCK ELSEIF_BLOCKS ENDIF
              ...
              |WHILE LB { printf("while("); } A_EXPN RB { printf("){\n"); } DO PROGRAM_STATEMENTS ENDWHILE { printf("}\n"); }
              |RERUN { printf("do{\n");  } PROGRAM_STATEMENTS UNTIL LB { printf("} while("); } A_EXPN RB { printf(");\n"); }
              |FOR LB { printf("for("); } VAR { strcpy(loop_var, yylval.var_name); printf(loop_var); }
               ...
               RB DO { printf("){\n");   }
               PROGRAM_STATEMENTS ENDFOR {  printf("}\n");}
              |VAR COLON { printf("%s:\n", yylval.var_name); }
              |COMMENT { char subtext[200];
	       		strncpy(subtext,&yylval.var_name[1],strlen(yylval.var_name)-2);
			printf("/*%s*/\n",subtext);}

IF_BLOCK: IF LB {   printf("if("); } A_EXPN RB { printf("){\n");   } PROGRAM_STATEMENTS {   printf("}\n"); }

ELSEIF_BLOCKS: ELSEIF_BLOCKS ELSEIF_BLOCK | ;
ELSEIF_BLOCK: ELSEIF LB { printf("else if("); } A_EXPN RB { printf("){\n"); } PROGRAM_STATEMENT { printf("\n}\n"); }
...
TERMINAL: VAR {
        ...
        | NUMBER { printf("%d", yylval.value); } | TEXT {printf("%s",yylval.text);}
